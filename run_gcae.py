		...
		for epoch in epochs:
			...
			if batch_size_project:
				...
				for b in range(n_train_batches):
					print(f'loop in n_train_batches: {b}. total n_train_batches {n_train_batches}')
					...
					print('decoded_train_batch:')
					print(decoded_train_batch.shape)
				...
				print('decoded_train after loop:')
				print(decoded_train.shape)
				print(decoded_train)
			...
			if train_opts["loss"]["class"] == "MeanSquaredError" and (data_opts["norm_mode"] == "smartPCAstyle" or data_opts["norm_mode"] == "standard"):
				...
				print('train: binarycrossentropy')
				...
				print('train: categoricalcrossentropy')
				print('decoded_train[:, 0:n_markers]')
				print(decoded_train[:, 0:n_markers].shape)
				genotypes_output = tf.cast(tf.argmax(alfreqvector(decoded_train[:, 0:n_markers]), axis = -1), tf.float16) * 0.5
				true_genotypes = targets_train
				print('genotypes_output:')
				print(genotypes_output)
				print('true_genotypes:')
				print(true_genotypes)
				genotype_concordance_metric.update_state(y_pred = genotypes_output[orig_nonmissing_mask], y_true = true_genotypes[orig_nonmissing_mask])
