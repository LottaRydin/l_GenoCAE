...
def make_input_hap(hap_output, dip_input):
	...
	alfreq = tf.keras.activations.sigmoid(hap_output)
	alfreq = tf.expand_dims(alfreq, -1)
	rounded_hap = tf.math.round(alfreq)

	div_mul = tf.constant(2., shape=dip_input.shape, dtype=tf.float64)
	diploid = tf.math.multiply(dip_input, div_mul)
